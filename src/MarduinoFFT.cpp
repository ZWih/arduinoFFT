...
double arduinoFFT::MajorPeak2(double a[]) {
  ...
  for (uint16_t i = 1; i < ((this->_samples >> 1) + 1); i++) {
    ...
    if ((this->_vReal[i - 1] < this->_vReal[i]) &&
        (this->_vReal[i] > this->_vReal[i + 1])) {
      if ((this->_vReal[i] > max2Y) && (IndexOfMaxY != i)) {
        max2Y = this->_vReal[i];
        IndexOfMax2Y = i;
      }
    }
  ...
  double delta =
      0.5 *
      ((this->_vReal[IndexOfMax2Y - 1] - this->_vReal[IndexOfMax2Y + 1]) /
       (this->_vReal[IndexOfMax2Y - 1] - (2.0 * this->_vReal[IndexOfMax2Y]) +
        this->_vReal[IndexOfMax2Y + 1]));
  double interpolatedX =
      ((IndexOfMax2Y + delta) * this->_samplingFrequency) / (this->_samples - 1);
  if (IndexOfMax2Y ==
      (this->_samples >> 1)) // To improve calculation on edge values
    interpolatedX =
...
double arduinoFFT::MajorPeak3(double a[]) {
  ...
  for (uint16_t i = 1; i < ((this->_samples >> 1) + 1); i++) {
    if ((this->_vReal[i - 1] < this->_vReal[i]) &&
        (this->_vReal[i] > this->_vReal[i + 1])) {
      if ((this->_vReal[i] > max3Y) && (IndexOfMaxY != i) && (IndexOfMax2Y != i)) {
        max3Y = this->_vReal[i];
        IndexOfMax3Y = i;
      }
    }
  ...
  double delta =
      0.5 *
      ((this->_vReal[IndexOfMax3Y - 1] - this->_vReal[IndexOfMax3Y + 1]) /
       (this->_vReal[IndexOfMax3Y - 1] - (2.0 * this->_vReal[IndexOfMax3Y]) +
        this->_vReal[IndexOfMax3Y + 1]));
  double interpolatedX =
      ((IndexOfMax3Y + delta) * this->_samplingFrequency) / (this->_samples - 1);
  if (IndexOfMax3Y ==
      (this->_samples >> 1)) // To improve calculation on edge values
    interpolatedX =
